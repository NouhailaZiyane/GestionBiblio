class OrdinateursController < ApplicationController
  ...
  def search1
    @ordinateur= Ordinateur.where(marque LIKE ?; %+ params[:q]+%)
    @ordinateur1= Ordinateur.where(description LIKE ?; %+ params[:q]+%)
  end
  ...
  private
    ...
    def ordinateur_params
      params.require(:ordinateur).permit(:marque; :description)
    end
end
