class AdherentsController < ApplicationController
  ...
  def search2
    @adherent= Adherent.where(nom LIKE ?; %+ params[:q]+%)
    @adherent1= Adherent.where(prenom LIKE ?; %+ params[:q]+%)
    @adherent2= Adherent.where(profession LIKE ?; %+ params[:q]+%)
  end
  ...
  private
    ...
    def adherent_params
      params.require(:adherent).permit(:nom; :prenom; :profession)
    end
end
